...
func TestCreateServiceEndpointCsmDuplicateID(t *testing.T) {
	rfID := "x3000c0r1b8"
	rfType := "UpdateService"
	const redfishURL = "x3001c0r0b0/redfish/v1/UpdateService"
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		t.Errorf("expected non-2xx on duplicate service endpoint %s %s, got HTTP %d", rfType, rfID, resp.StatusCode)
	}
}
